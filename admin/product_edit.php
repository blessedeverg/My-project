...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare("UPDATE products SET name = ?, description = ?, price = ?, category_id = ?, 
                                 image_url = ?, display_location = ?, status = ?, updated_at = NOW() 
                                 WHERE id = ?");
            
            if ($stmt->execute([$name, $description, $price, $category_id, $image_url, $display_location, $status, $product_id])) {
                // Update product_images table if image changed
                if ($image_url !== $product['image_url']) {
                    // Delete old image records
                    $stmt = $db->prepare("DELETE FROM product_images WHERE product_id = ?");
                    $stmt->execute([$product_id]);
                    
                    // Add new image record if there's an image
                    if ($image_url) {
            ...
            <div class="content-area">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="btn-group">
                            <a href="product_view.php?id=<?php echo $product['id']; ?>" class="btn btn-info btn-sm">
                    ...
                    <div class="card-body">
                        <form method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="display_location" name="display_location" class="form-control">
                                        <option value="homepage" <?php echo ($product['display_location'] === 'homepage') ? 'selected' : ''; ?>>Homepage</option>
                                        <option value="featured" <?php echo ($product['display_location'] === 'featured') ? 'selected' : ''; ?>>Featured Products</option>
                                        <option value="category_page" <?php echo ($product['display_location'] === 'category_page') ? 'selected' : ''; ?>>Category Page Only</option>
                                        <option value="all_products" <?php echo ($product['display_location'] === 'all_products') ? 'selected' : ''; ?>>All Products Page</option>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="file-upload">
                                        ...
                                        <div class="file-upload-btn">
                                            ...
                                            <?php echo $product['image_url'] ? 'Change Image' : 'Choose Image File'; ?>
                                        </div>
                                    </div>
                            ...
                            <div style="margin-top: 2rem; padding-top: 2rem; border-top: 1px solid #e9ecef;">
                                ...
                                <a href="product_view.php?id=<?php echo $product['id']; ?>" class="btn btn-info">
                                    <i class="fas fa-eye"></i>
                                    View Product
                                </a>
                            </div>
